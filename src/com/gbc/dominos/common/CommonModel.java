...
public class CommonModel {
    ...
    public static String FormatResponseEx(int msgType, String msgName, JsonElement jsonEle) {
        JsonObject json = new JsonObject();
        json.addProperty(DefineMessage.MsgType.PROPERTY_NAME, msgType);
        json.addProperty(DefineMessage.MsgName.PROPERTY_NAME, msgName);
        if (jsonEle != null) {
            json.add("dt", jsonEle);
        }
    ...
    public static String FormatResponseVRF(int msgType, String msgName, JsonElement jsonEle) {
        JsonObject json = new JsonObject();
        json.addProperty(DefineMessage.MsgType.PROPERTY_NAME, msgType);
        json.addProperty(DefineMessage.MsgName.PROPERTY_NAME, msgName);
        if (jsonEle != null) {
            //json.add("dt", jsonEle);
            String datax = _gson.toJson(jsonEle);
            json.addProperty("dt", datax);
            String token = HMACUtil.HMacHexStringEncode("HmacSHA256", AppConst.HMAC_SHA256_KEY_2, datax);
            json.addProperty("tk", token);
        }
        return _gson.toJson(json);
    }
}
